...
def lambda_handler(event, context):
    ...
    keyout = 'NN/' + companyname + '/KMeansCentroids/' + stp    + '/' + I +'.csv'
    ...
    MatrixDate=TickCSVtoMatrixlmbda(bucketin, keydate)
    ...
    data=prepdata(MatrixField,MatrixOS, MatrixDate)
    ...
    MatrixToCSVWritelmbda(bucketout, pi,keyout)
...
def prepdata(TrainingData,TrainingosData, DateData):
    ...
    DateData=DateData[1:lend]
    mntdiffdata=[]
    mnt=15
    for id in range(len(DateData)):
        d=str.split(DateData[id],',')
        if(float(d[3])<mnt):mnt=mnt-60
        mntdiff=float(d[3])-mnt
        mnt=float(d[3])
        mntdiffdata.append(mntdiff)
    mntdiffsort=BubbleSort(mntdiffdata)
    med=CalculationForMedian(mntdiffsort)
    vrn=CalculateStandardDeviation(mntdiffsort,med)
...
def MatrixToCSVWritelmbda(bucket, PI,key):
